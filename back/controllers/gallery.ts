...
export const getAllImages = async (
  ...
  const pageNumber: number = parseInt(String(req.query.page), 10) || 0;
...
export const updateImageInfo = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  try {
    ...
    const existingImage: any | null = await ImageSchema.findById(imageId);

    if (!existingImage) {
      return res.status(404).json({ error: 'Image not found' });
    }

    const updatedImage = await ImageSchema.findOneAndUpdate(
      { _id: imageId },
      req.body,
      { new: true }
    );
